...
def main():
    ...
    * { box-sizing: border-box; }
    ...
    // Chart 3: top taxonomy items
    function renderTopItems() {
      const depth = document.getElementById('d3-depth').value; // 1 | 2 | 3
      const substance = document.getElementById('d3-substance').value;
      ...
      if (substance !== 'all') items = items.filter(c => c.substance === substance);
      if (onlyScene) items = items.filter(c => c.is_scene_level);

      // If consensus, keep only items that pass AGREE on some shared scene, OR trip-level
      // where both raters attached the same item
      if (consensus) {
        // Build AGREE pairs
        const agreePairs = new Set();
        DATA.flags.forEach(f => { if (f.agreement === 'AGREE') agreePairs.add(f.scene_id + '|' + f.item_id); });
        // Trip-level consensus: both raters attached at trip-level
      ...
      render('chart-items', {
        ...
        data: {
          ...
          datasets: [{ label: 'count', data: top.map(t => t[1]), backgroundColor: '#607d8b' }],
        },
        options: {
          indexAxis: 'y',
          responsive: true, maintainAspectRatio: false,
    ...
    function init() {
      ...
      // all filter selects
      ['d1-substance','d2-substance','d3-substance','d4-substance','d5-substance'].forEach(id => {
        const el = document.getElementById(id);
        el.addEventListener('change', rerender);
      });
      ['d1-groupby','d1-showmode','d3-depth','d3-consensus','d3-onlyscene','d4-metric']
    ...
    doc = [
        ...
        '<h2>Top 20 taxonomy items</h2>',
        '<p class="desc">Most-coded taxonomy items. Choose depth (section / category / leaf), '
        'restrict to scene-level only, and toggle the consensus filter (only items both raters attached).</p>',
        '<div class="filters">',
        ...
        '</select></label>',
        '<label>Substance <select id="d3-substance">'
        '<option value="all">all</option>'
        '<option value="brugmansia">brugmansia</option>'
        '<option value="psilocybin">psilocybin</option>'
        '</select></label>',
